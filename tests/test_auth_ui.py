...
def test_challenge_demo_login_is_disabled_by_default(monkeypatch) -> None:
    monkeypatch.delenv("RUHU_CHALLENGE_DEMO_LOGIN_ENABLED", raising=False)
    monkeypatch.delenv("RUHU_CHALLENGE_DEMO_PASSWORD", raising=False)
    ...
    async def run() -> None:
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
            response = await client.post(
                "/auth/challenge-demo/login",
                json={"email": "judge@ruhu.ai", "password": "secret"},
            )
...
def test_challenge_demo_login_issues_browser_session(monkeypatch) -> None:
    monkeypatch.setenv("RUHU_CHALLENGE_DEMO_LOGIN_ENABLED", "true")
    monkeypatch.setenv("RUHU_CHALLENGE_DEMO_EMAIL", "judge@ruhu.ai")
    monkeypatch.setenv("RUHU_CHALLENGE_DEMO_PASSWORD", "demo-password")
    ...
    async def run() -> None:
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
            bad = await client.post(
                "/auth/challenge-demo/login",
                json={"email": "judge@ruhu.ai", "password": "wrong"},
            )
            good = await client.post(
                "/auth/challenge-demo/login",
                json={"email": "judge@ruhu.ai", "password": "demo-password"},
            )

        assert bad.status_code == 401
        assert good.status_code == 200
        payload = good.json()
        assert payload["user"]["email"] == "judge@ruhu.ai"
        assert payload["organization"]["role"] == "admin"
        assert "access_token=" in good.headers.get("set-cookie", "")
        assert auth_service.identity_store.get_user_by_email("judge@ruhu.ai") is not None
