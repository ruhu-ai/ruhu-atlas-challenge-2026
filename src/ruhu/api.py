...
def create_app(
    ...
    *,
    ...
    def _seed_challenge_demo_agent(organization_id: str) -> None:
        template_path = (
            Path(__file__).resolve().parent
            / "templates"
            / "system"
            / "nafmfb-financial-support-assistant-demo.json"
        )
        try:
            payload = json.loads(template_path.read_text(encoding="utf-8"))
            agent_document = AgentDocument.model_validate(payload["agent_document"])
            agent_settings = dict(payload.get("default_agent_settings") or {})
            agent_settings["source_template_id"] = payload.get("template_id")
            agent_registry.create_agent_document(
                agent_id="nafmfb_financial_support_assistant_demo",
                agent_name="NAFMFB Financial Support Assistant - Demo",
                document=agent_document,
                settings={"agent_settings": agent_settings},
                organization_id=organization_id,
            )
        except ValueError as exc:
            if "agent already exists" not in str(exc):
                logger.warning("challenge demo agent seeding failed", exc_info=True)
        except Exception:
            logger.warning("challenge demo agent seeding failed", exc_info=True)
    ...
    if auth_resolver is not None and effective_auth_service is not None:
        ...
        app.include_router(
            build_auth_sessions_router(
                ...
                demo_agent_seeder=_seed_challenge_demo_agent,
            )
        )
