...
class AuthService:
    ...
    def issue_demo_browser_session(
        self,
        *,
        email: str,
        display_name: str | None = None,
        ...
        """Provision a demo user and issue a normal browser session.

        This is intentionally not password auth. The caller owns any gating
        policy and invokes this only for explicitly enabled demo deployments.
        """
        normalized_email = normalize_email(email)
        ...
        memberships = self.identity_store.list_organization_memberships_for_user(user.user_id)
        if not memberships:
            raise AuthorizationError("demo user is not a member of an organization")
