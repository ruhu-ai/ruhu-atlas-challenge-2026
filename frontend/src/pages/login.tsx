...
import { Mail, Key, ArrowLeft, Lock } from 'lucide-react'
...
export default function LoginPage() {
  ...
  const [demoEmail, setDemoEmail] = useState(import.meta.env.VITE_CHALLENGE_DEMO_EMAIL || '')
  const [demoPassword, setDemoPassword] = useState('')
  const [isLoading, setIsLoading] = useState(false)
  const [loadingProvider, setLoadingProvider] = useState<'google' | 'magic' | 'sso' | 'demo' | null>(null)
  const [error, setError] = useState<string | null>(null)
  const challengeDemoLoginEnabled = import.meta.env.VITE_CHALLENGE_DEMO_LOGIN === 'true'
  ...
  const handleChallengeDemoSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    clearError()
    if (!demoEmail.trim() || !demoPassword) return
    setLoadingProvider('demo')
    try {
      await authService.challengeDemoLogin(demoEmail.trim(), demoPassword)
      redirectTo('/dashboard')
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Invalid demo credentials')
      setLoadingProvider(null)
    }
  }
          ...
          <CardContent>
            ...
            {step.kind === 'main' && (
              <div className="space-y-2">
                {challengeDemoLoginEnabled && (
                  <form onSubmit={handleChallengeDemoSubmit} className="space-y-3 rounded-md border border-border bg-muted/25 p-3">
                    <div className="flex items-center gap-2 text-sm font-medium text-foreground">
                      <Lock className="h-4 w-4" aria-hidden="true" />
                      <span>Challenge demo access</span>
                    </div>
                    <div className="space-y-2">
                    ...
                    <Button
                      type="submit"
                      className="w-full"
                      disabled={!demoEmail.trim() || !demoPassword || anyLoading}
                      isLoading={loadingProvider === 'demo'}
                    >
                      Sign in to demo
                    </Button>
                  </form>
